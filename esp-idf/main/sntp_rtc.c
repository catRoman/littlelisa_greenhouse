...
void sntp_rtc_init(void){
    ...
    sntp_rtc_server_connection_check();
      ...
      if((err[0] = esp_netif_sntp_start()) == ESP_OK){
            ESP_LOGI(TAG, "SNTP started");
                ...
                if(sntp_get_sync_status() == SNTP_SYNC_STATUS_COMPLETED){
                    ...
                    ESP_LOGI(TAG, "current sntp server set to: %s at %s", sntp_getservername(0), ipaddr_ntoa(esp_sntp_getserver(0)));
                }
...
void sntp_rtc_system_test(){
    ...
    xTaskCreatePinnedToCore(sntp_rtc_system_test_task, "sntp_system_test", 4096, NULL, 5,NULL, 1);
}
...
void sntp_rtc_server_connection_check(){
    ESP_LOGI(TAG, "starting time check");
    xTaskCreatePinnedToCore(sntp_rtc_server_connection_check_task, "sntp_connection_check", 2048, NULL, 5,NULL, 1);
}
