...
void register_http_server_handlers(void)
{
    ESP_LOGI(HTTP_HANDLER_TAG, "http_server_configure: Registering URI handlers");
        ...
        /* Register a generic preflight handler */
            ...
            httpd_register_uri_handler(http_server_handle, &ws_echo);
            ...
            httpd_register_uri_handler(http_server_handle, &ws_sensor);
}
...
esp_err_t ws_echo_handler(httpd_req_t *req)
{
    if (req->method == HTTP_GET) {
        ESP_LOGI(HTTP_HANDLER_TAG, "Handshake done, the new echo websocket connection was opened");
        return ESP_OK;
    }
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(HTTP_HANDLER_TAG, "httpd_ws_recv_frame failed to get frame len with %d", ret);
        return ret;
    }
    ESP_LOGI(HTTP_HANDLER_TAG, "frame len is %d", ws_pkt.len);
    if (ws_pkt.len) {
        /* ws_pkt.len + 1 is for NULL termination as we are expecting a string */
        buf = calloc(1, ws_pkt.len + 1);
        if (buf == NULL) {
            ESP_LOGE(HTTP_HANDLER_TAG, "Failed to calloc memory for buf");
        ...
        if (ret != ESP_OK) {
            ESP_LOGE(HTTP_HANDLER_TAG, "httpd_ws_recv_frame failed with %d", ret);
            free(buf);
            return ret;
        }
        ESP_LOGI(HTTP_HANDLER_TAG, "Got packet with message: %s", ws_pkt.payload);
    }
    ESP_LOGI(HTTP_HANDLER_TAG, "Packet type: %d", ws_pkt.type);
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(HTTP_HANDLER_TAG, "httpd_ws_send_frame failed with %d", ret);
    }
    free(buf);
    return ret;
}
...
{
    if (req->method == HTTP_GET) {
        ESP_LOGI(HTTP_HANDLER_TAG, "Handshake done, the new sensor websocket connection was opened");
    ...
    for(int i = 0; i< 10; i++){
         ret  = httpd_ws_send_frame(req, &ws_pkt);
            if (ret != ESP_OK) {
                ...
                ESP_LOGI(HTTP_HANDLER_TAG, "packet sent");
            }
