...
void websocket_send_sensor_data_queue(void *vpParameter)
{
    ...
    for (;;)
    {
        if (xQueueReceive(websocket_send_sensor_data_queue_handle, &ws_frame_data, portMAX_DELAY))
        {
            ...
            // ESP_LOGW("ws-start", "free min size:%lu", esp_get_free_heap_size());
            for (int j = 0; j < num_websocket_sensor_clients; j++)
            {
                ...
                // ESP_LOGW("ws-start-1", "free min size:%lu", esp_get_free_heap_size());
                esp_err_t ret = httpd_ws_send_frame_async(websocket_server_handle, websocket_sensor_clients->items[j], ws_frame_data.ws_pkt);

                // ESP_LOGW("ws-start-2", "free min size:%lu", esp_get_free_heap_size());
                //  ret  = httpd_ws_send_frame(req, &ws_pkt);
            ...
            // ESP_LOGW("ws-start-7", "free min size:%lu", esp_get_free_heap_size());
            free(ws_frame_data.ws_pkt->payload);
            // ESP_LOGW("ws-mem", "freed at %p", ws_frame_data.ws_pkt->payload);
            ws_frame_data.ws_pkt->payload = NULL;
            // }
            // ESP_LOGW("ws-start-9", "free min size:%lu", esp_get_free_heap_size());
        }
        // ESP_LOGW("ws-start-end", "free min size:%lu", esp_get_free_heap_size());
        taskYIELD();
    }
}
