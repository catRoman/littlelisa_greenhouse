(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))n(s);new MutationObserver(s=>{for(const r of s)if(r.type==="childList")for(const c of r.addedNodes)c.tagName==="LINK"&&c.rel==="modulepreload"&&n(c)}).observe(document,{childList:!0,subtree:!0});function o(s){const r={};return s.integrity&&(r.integrity=s.integrity),s.referrerPolicy&&(r.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?r.credentials="include":s.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function n(s){if(s.ep)return;s.ep=!0;const r=o(s);fetch(s.href,r)}})();document.querySelector(".network-info");document.querySelector(".sensor-data-node-box");document.querySelector(".section-sensor-data");const G=document.querySelector(".log-output"),A=document.querySelector(".log-refresh"),N=document.querySelector(".sensor-refresh");let a,l;document.querySelector(".ota-log");const x=document.querySelector(".ota-status"),h=document.querySelector(".ota-status-info"),y=document.querySelector(".ota-status-reset"),X=document.getElementById("fileInput");let q=[],E={};const C=new Set(null);let m="node",S;const T=document.getElementById("uploadBtn"),J=document.getElementById("uploadForm");J.addEventListener("submit",async e=>{e.preventDefault(),T.disabled=!0;const t=X.files[0],o="ota/update_prop";x.classList.toggle("hidden");try{const n=await fetch(o,{method:"POST",headers:{"Content-Type":"application/octet-stream"},body:t});if(n.ok){const s=await n.text();console.log(s),h.textContent="Upload Complete..."}else throw new Error(`Status: ${n.status}`)}catch(n){console.error("Error uploading file:",n),setTimeout(()=>{h.textContent="SD save failed...",y.textContent=n},1e4),x.classList.toggle("hidden"),T.disabled=!1}});function I(){a!==void 0&&(a.close(),console.log("refreshing log socket..."),G.value="Refreshing esp log stream...",setTimeout(()=>_(S),3e3))}A.addEventListener("click",e=>{I()});A.addEventListener("touchend",e=>{e.preventDefault(),I()});function K(){l!==void 0&&(l.close(),setTimeout(()=>$(S),3e3))}N.addEventListener("click",e=>{K()});N.addEventListener("touchend",e=>{e.preventDefault(),sensorRefreshEv,ent()});const v=document.querySelector(".icon-open"),k=document.querySelector(".icon-close"),L=document.querySelector(".menu"),g=document.querySelector("main");document.querySelector(".nav-btn.close");const j=document.querySelectorAll(".nav-btn");function O(e,t){e.stopPropagation;const o=[...t];switch(o[o.length-1]){case"dev_btn":u(".device-info");break;case"net_btn":u(".network-info");break;case"db_btn":u(".sd-db-info");break;case"sys_btn":u(".system-health");break;case"ota_btn":S.module_info.type=="controller"?u(".ota-update"):console.log("Disabled for nodes. TODO: only render button for controller");break;case"close":v.classList.toggle("hidden"),g.classList.toggle("hidden"),L.classList.toggle("hidden"),g.style.pointerEvents="none",setTimeout(()=>{g.style.pointerEvents="auto"},100);break}}j.forEach(e=>{e.addEventListener("click",function(t){O(t,this.classList)})});j.forEach(e=>{e.addEventListener("touchend",function(t){t.preventDefault(),O(t,this.classList)})});function M(){v.classList.toggle("hidden"),g.classList.toggle("hidden"),L.classList.toggle("hidden")}v.addEventListener("click",e=>{M()});v.addEventListener("touchend",e=>{e.preventDefault(),M()});function u(e){console.log(e),L.classList.toggle("hidden"),document.querySelector(e).classList.toggle("hidden"),k.classList.toggle("hidden")}function B(){const e=document.querySelector(".menu-select");Array.from(e.children).forEach(t=>{!t.classList.contains("hidden")&&!t.classList.contains("head-icon")&&(console.log(t.classList[0]),u(`.${t.classList[0]}`))})}k.addEventListener("click",e=>{B()});k.addEventListener("touchend",e=>{e.preventDefault(),B()});function Q(e){const t=document.querySelector(`${e} > .sensor-summary`);t.addEventListener("click",function(o){(o.target===this||this.contains(o.target))&&(t.nextElementSibling.classList.toggle("hidden"),t.querySelector(".node-title").classList.toggle("green"))})}async function R(){try{const e=await fetch("/api/envState");if(!e.ok)throw new Error("Network response was not ok");const t=await e.json();E=Object.values(t),U()}catch(e){console.log(e.message)}}async function Y(e){try{const t=await fetch("/api/envStateUpdate",{method:"PUT",body:e});if(!t.ok)throw new Error("Network response was not ok");const o=await t.json();E=Object.values(o),U()}catch(t){console.log(t.message)}}function U(){const e=document.querySelector(".env-cntrl");e.innerHTML="",E.forEach(o=>{e.insertAdjacentHTML("beforeend",` <li class="env-li" >
      ...
      <button data-id="${o.id}" class="env-status ${o.state==="on"?"env-status-on":"env-status-off"}">${o.state}</button>
        ...
        >`)}function P(e,t){const{module_info:{type:o,location:n,identifier:s},sensor_list:r}=t;document.querySelector(`.${e}`)!==null?document.querySelector(`.${e}`):document.querySelector(".section-sensor-data").insertAdjacentHTML("beforeend",` <!-- NODE- ${e}-->
  ...
  </li>`)}),Q(`.${e}`)}function ce(e){const{ap_ssid:t,ap_channel:o,ap_pass:n,ap_max_connect:s}=e;document.querySelector(".network-info > ul").insertAdjacentHTML("beforeend",`
  <li class="ap-info">
