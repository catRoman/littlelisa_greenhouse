...
void nvs_set_sensor_arr(const int8_t *sensor_arr, int8_t arrLength)
{
    ...
    ESP_ERROR_CHECK(nvs_set_blob(nvs_sensor_arr_handle, NVS_SENSOR_ARR_INDEX, sensor_arr, arrLength * sizeof(int8_t)));
...
void nvs_set_env_state_arr(Env_state_t *state_arr_gt, int8_t arrLength)
{
    ...
    ESP_ERROR_CHECK(nvs_set_blob(nvs_env_state_arr_handle, NVS_ENV_STATE_CONFIG_ARR_INDEX, state_arr_gt, arrLength * sizeof(Env_state_t)));
