...
`+r}n&&(s.scrollTop=s.scrollHeight)}function ce(e){let t=e.substring(e.indexOf("("));return t=t.substring(0,t.indexOf("[")-1),t}function p(e){return"node-"+e.replaceAll(":","-")}function re(e){const t=/.*UPDATING_NODE_(\d+).*/,o=e.match(t);o&&(y.textContent="Updating",h.textContent=`Updating node ${o[1]}...`,console.log(`Updating node ${o[1]}...`))}window.addEventListener("resize",()=>{console.log(`Viewport Width: ${window.innerWidth}, Viewport Height: ${window.innerHeight}`)});console.log(`Initial Viewport Width: ${window.innerWidth}, Initial Viewport Height: ${window.innerHeight}`);G();z();W();O();D();setInterval(()=>{m==="controller"&&I()},5e3);setInterval(O,5e3);setInterval(D,5e3);
